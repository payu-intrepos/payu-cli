...
def save_profile(
    ...
    *,
    ...
    if auth_token:
        stored_token = _keyring_set(KEYRING_SERVICE, f"{profile}:auth_token", auth_token)
        if not stored_token:
            profiles[profile]["auth_token"] = auth_token
            console.print(
                "[yellow]⚠  keyring unavailable — auth_token stored in plaintext config[/yellow]"
            )
...
def load_profile(profile: Optional[str] = None) -> dict:
    ...
    auth_token = (
        _keyring_get(KEYRING_SERVICE, f"{name}:auth_token")
        or p.get("auth_token")
...
def delete_profile(profile: str) -> None:
    ...
    _keyring_delete(KEYRING_SERVICE, f"{profile}:auth_token")
